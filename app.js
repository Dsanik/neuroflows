...
try {
  ...
  if (v1raw && !localStorage.getItem(KEY)) {
    const v1 = JSON.parse(v1raw);
    state = { ...state, ...v1 };
    localStorage.setItem(KEY, JSON.stringify(state));
    localStorage.removeItem(V1_KEY);
    console.log('[NeuroFlow] Migrated data from v1 to v2');
  }
} catch(e) { console.error('[NeuroFlow] Migration failed:', e); }

const cloud = {
  set: (k, v) => new Promise(res => { if (!tg?.CloudStorage) return res(); tg.CloudStorage.setItem(k, v, () => res()); }),
  get: (k) => new Promise(res => { if (!tg?.CloudStorage) return res(null); tg.CloudStorage.getItem(k, (err, v) => res(err ? null : v)); }),
};

// Loads state from CloudStorage (cross-device sync) with localStorage as fallback/mirror.
// Must be awaited once at app startup before rendering, since CloudStorage reads are async.
async function loadState() {
  ...
  } catch(e) { console.error('[NeuroFlow] loadState failed:', e); }
}

const saveStore = () => {
  const s = JSON.stringify(state);
  localStorage.setItem(KEY, s);
  if (tg?.CloudStorage) cloud.set(KEY, s);
};
...
const store = {
  ...
  resetAll: () => {
    ...
    if (tg?.CloudStorage) tg.CloudStorage.removeItem(KEY);
    location.reload();
  },
...
function recalcAverageCycle() {
  ...
  for (let i = 1; i < periodDates.length; i++) {
    ...
    if (diff > 1) {
      cycleStarts.push({ start: new Date(currentStart), end: new Date(periodDates[i-1]), next: curr });
      currentStart = periodDates[i];
    }
  }
  const lengths = cycleStarts.map(c => Math.round((c.next - c.start) / 86400000));
  const valid = lengths.filter(l => l >= 21 && l <= 35);
  if (valid.length > 0) {
    const avg = Math.round(valid.reduce((a, b) => a + b, 0) / valid.length);
    if (avg !== profile.averageCycleLength) {
      profile.averageCycleLength = avg;
    }
  }
}
